...
function ClassPage({}: Props) {
  ...
  const [classDataFilter, setClassDataFilter] = useState<ClassData[]>([]);
  ...
  const getClass = () => {
    axios
      .get("http://54.252.240.166/classes", {
        headers: {
      ...
      .then((response) => {
        ...
        setClassDataFilter(response?.data?.data);
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  const deleteClass = (id: number) => {
    axios
      .delete(`http://54.252.240.166/classes/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const handleSearch = () => {
    ...
    const filterData = classData.filter(item => item.class_name.toLowerCase().indexOf(search.toLowerCase()) !== -1)
    
    setClassDataFilter(filterData)
  }
        ...
        <main className="flex-grow flex flex-col">
          ...
          <div className="flex-grow bg-white p-4 m-3 rounded-md">
            ...
            <table className="w-full border-collapse border">
              ...
              <tbody>
                {classDataFilter?.map((item, index) => (
                  <tr>
            ...
            <div className="flex justify-end mt-4 space-x-3">
              <button className="px-4 py-2 bg-gray-400 text-white rounded-md shadow-lg hover:bg-gray-500 focus:outline-none">
                Prev
              </button>
              <button className="px-4 py-2 bg-gray-400 text-white rounded-md shadow-lg hover:bg-gray-500 focus:outline-none">
                Next
